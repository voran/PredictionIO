...
package org.apache.predictionio.data.storage.kudu
...
object HBEventsUtil {
  ...
  def eventToPut(event: Event, table: KuduTable, appId: Int): (Insert, RowKey) = {
    ...
    row.addString(colNames("event"), event.event)
    row.addString(colNames("entityType"), event.entityType)
    row.addString(colNames("entityId"), event.entityId)

    event.targetEntityType.foreach { targetEntityType =>
      row.addString(colNames("targetEntityType"), event.targetEntityType)
    }

    event.targetEntityId.foreach { targetEntityId =>
      row.addString(colNames("targetEntityId"), targetEntityId)
    }


    // TODO: make properties Option[]
    if (!event.properties.isEmpty) {
      row.addString(colNames("properties"), write(event.properties.fields))
    }

    event.prId.foreach { prId =>
      row.addString(colNames("prId"), prId)
    }

    row.addString(colNames("eventTime", event.eventTime.getMillis)
    val eventTimeZone = event.eventTime.getZone
    if (!eventTimeZone.equals(EventValidation.defaultTimeZone)) {
      row.addString(colNames("eventTimeZone"), eventTimeZone.getID)
    }

    row.addLong(colNames("creationTime"), event.creationTime.getMillis)
  ...
  def resultToEvent(result: RowResult, appId: Int): Event = {
